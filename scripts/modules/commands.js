...
import { displayBanner, displayHelp, displayNextTask, displayTaskById, displayComplexityReport, getStatusWithColor } from "./ui.js";
...
function registerCommands(programInstance) {
  ...
  programInstance
    ...
    .option("--research", "Enable Gemini AI for research-backed subtask generation")
    ...
    .action(async (options) => {
      ...
      if (allFlag) {
        ...
        if (useResearch) {
          console.log(chalk.blue("Using Gemini AI for research-backed subtask generation"));
        } else {
  ...
  programInstance
    ...
    .action(async (options) => {
      ...
      await analyzeComplexityCommand(options);
    });
...
export async function analyzeComplexityCommand(options) {
  try {
    displayBanner();
