...
const testListTasks = (tasksData, statusFilter, withSubtasks = false) => {
  ...
  const filteredTasks = statusFilter
    ? tasksData.tasks.filter((task) => task.status && task.status.toLowerCase() === statusFilter.toLowerCase())
    : tasksData.tasks;
...
describe("Task Manager Module", () => {
  ...
  describe.skip("analyzeTaskComplexity function", () => {
    ...
    test("should handle missing tasks in the response (Gemini)", async () => {
      ...
      expect(mockWriteJSON).toHaveBeenCalledWith(
        reportPath,
        expect.objectContaining({
          tasks: expect.arrayContaining([expect.objectContaining({ id: 1 }), expect.objectContaining({ id: 2 }), expect.objectContaining({ id: 3 })]),
        })
      );
    });
  });
  ...
  describe("listTasks function", () => {
    ...
    test("should filter tasks by status when filter is provided", async () => {
      ...
      // Assert
      expect(result.filteredTasks.length).toBe(testTasksData.tasks.filter((t) => t.status === statusFilter).length);
  ...
  describe("addSubtask function", () => {
    ...
    test("should throw an error if parent task does not exist", async () => {
      ...
      expect(() => testAddSubtask("tasks/tasks.json", 999, null, newSubtaskData)).toThrow(/Parent task with ID 999 not found/);
    ...
    test("should throw an error if existing task does not exist", async () => {
      ...
      expect(() => testAddSubtask("tasks/tasks.json", 1, 999, null)).toThrow(/Task with ID 999 not found/);
  ...
  describe("removeSubtask function", () => {
    ...
    test("should throw an error if subtask ID format is invalid", async () => {
      ...
      expect(() => testRemoveSubtask("tasks/tasks.json", "1", false)).toThrow(/Invalid subtask ID format/);
    ...
    test("should throw an error if parent task does not exist", async () => {
      ...
      expect(() => testRemoveSubtask("tasks/tasks.json", "999.1", false)).toThrow(/Parent task with ID 999 not found/);
    ...
    test("should throw an error if subtask does not exist", async () => {
      ...
      expect(() => testRemoveSubtask("tasks/tasks.json", "1.999", false)).toThrow(/Subtask 1.999 not found/);
